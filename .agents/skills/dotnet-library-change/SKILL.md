...
description: Use esta skill ao alterar codigo, API publica, compatibilidade, testes ou empacotamento da biblioteca Dapper.TypedParameters.SqlServer. Nao use para tarefas puramente documentais, CI puro ou publicacao real de pacote.
---

# Objetivo

Orientar alteracoes pequenas e seguras na biblioteca `Dapper.TypedParameters.SqlServer`, preservando API publica, compatibilidade e conteudo do pacote NuGet.
...
- Trate mudanca de assinatura publica, namespace, tipo exposto, excecao observavel ou comportamento de `SqlParameter` como potencial breaking change.
...
- Use somente `Microsoft.Data.SqlClient`.
- Mantenha `PackageReference` sem `Version=` e altere versoes somente em `Directory.Packages.props`.

# Processo

1. Leia `AGENTS.md`, `README.md`, ADRs relevantes em `docs/decisions/`, o `.csproj` da biblioteca e os testes proximos.
...
8. Para `SqlParameter`, valide tipo SQL, `Value`, `DBNull.Value`, `Size`, `Precision`, `Scale`, direcao e reutilizacao de parametro quando aplicavel.
9. Prefira testes unitarios para contratos puros e testes de integracao somente quando o comportamento exigir SQL Server real.
10. Nao altere testes apenas para faze-los passar.
11. Ao alterar contrato ou decisao arquitetural, atualize `README.md` ou ADR.
12. Antes de finalizar, valide restore, build, test e pack.
...
- O pacote passou a expor dependencia nova?
