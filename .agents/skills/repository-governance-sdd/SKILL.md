...
description: Use esta skill para tarefas SDD de governanca do repositorio Dapper.TypedParameters, criacao ou revisao de skills Codex, AGENTS.md, ADRs, prompts e documentacao de processo. Nao use para implementar codigo de producao ou testes da biblioteca.
---
...
Use esta skill para evoluir orientacoes de agentes, skills, ADRs e convencoes operacionais sem aumentar desnecessariamente o contexto.
...
- Avaliar se uma decisao deve virar ADR em `docs/decisions/` ou ficar em documentacao operacional.
- Revisar documentacao de processo, standards, onboarding ou convencoes do repositorio.

# Quando nao usar

- Implementar comportamento em `Dapper.TypedParameters.SqlServer`.
- Alterar testes da biblioteca.
- Corrigir pipelines ou empacotamento sem trabalho de governanca/documentacao.
...
- Skills focadas, com `SKILL.md` e frontmatter valido quando aplicavel.
...
9. Evite duplicar no `AGENTS.md` fluxos longos que pertencem ao corpo de uma skill.
...
- Confirme ausencia de referencias a microservicos, APIs HTTP, Kafka, EF Core, Terraform ou infraestrutura inexistente.
...
- Nao alterar codigo de producao nem testes da biblioteca.
- Nao alterar pipeline salvo necessidade explicita e justificada.
- Nao fazer push nem criar branch sem pedido explicito.
